...
ctx.stroke();startA()},recentTracks=()=>{fetch("https://janus.shoutca.st/recentfeed/kuronegai/json/").then(res=>res.json()).then(data=>{for(let i=1;i<5;i++){try{let track=clearStr(data.items[i].title);document.getElementById("title"+i).textContent=track[1];document.getElementById("artist"+i).textContent=track[0];if(data.items[i].enclosure.url.indexOf("nocover.png")>0)document.getElementById("cover"+i).setAttribute("src","https://2.bp.blogspot.com/-thb9iU60J7M/W4TKqJR2rQI/AAAAAAAAAVQ/WrvAIx7N5sQh1maEq2x9I0o9L5PjnahQwCLcBGAs/s1600/no_cover_60x60.jpg");else document.getElementById("cover"+i).setAttribute("src",data.items[i].enclosure.url)}catch{document.getElementById("title"+i).textContent="Sin titulo";document.getElementById("artist"+i).textContent="Sin Artista";document.getElementById("cover"+i).setAttribute("src","https://2.bp.blogspot.com/-thb9iU60J7M/W4TKqJR2rQI/AAAAAAAAAVQ/WrvAIx7N5sQh1maEq2x9I0o9L5PjnahQwCLcBGAs/s1600/no_cover_60x60.jpg")}}}).catch(error=>alert(error+" Temas Recientes No Cargados"))},clearStr=(str)=>{str=str.replace(" - "," \\ ").split("\\");if(str.length>1){if(str[1].charAt(0)===' ')str[1]=str[1].substr(1);if(str[0].charAt(str[0].length-1)===' ')str[0]=str[0].slice(0,-1)}else{let temp=str[0];str[1]=temp;str[0]="Desconocido"}
